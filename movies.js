...
// Get the max revenue of the max revenues
const maxRevenue = data.map(x => x.movies.reduce((a, b) => Math.max(a, b.revenue), 0))
                       .reduce((a, b) => Math.max(a, b));

// Scale everything to fit to 600px wide
...
// Draw a week's chart
function draw(step) {
    // Function to calculate how far down an element is
    const elementTop = (_, i) => 45 * i + "px";

    // Update the title with the week of the currently displayed data
    d3.select(".title")
      .text(data[step].daterange);

    // Select all chart elements with key function
    const chart = d3.select(".chart")
                    .selectAll("div")
                    .data(data[step].movies, a => a.name);
    ...
    merged.append("div")
          .text(d => d.name + ": " + d.displayRevenue);

    // Generate the bar for a movie
    merged.append("div")
          .attr("class", "bar")
    ...
    if (step + 1 < data.length) {
        window.setTimeout(draw, 900, step + 1);
    }
}

// Draw the first week's chart when the page loads
window.onload = () => {
    draw(0);
}
